...
california_yearly <- dangerous_chemicals %>%
  group_by(Year) %>%
  summarize(Total_Usage = sum(Value, na.rm = TRUE))


# Line plot for yearly usage
ggplot(california_yearly, aes(x = Year, y = Total_Usage)) +
  geom_line(color = "blue", size = 1) +
  geom_point(size = 2) +
  labs(title = "Yearly Pesticide Usage in California",
       x = "Year",
       y = "Total Usage (Value)") +
  theme_minimal()
```
